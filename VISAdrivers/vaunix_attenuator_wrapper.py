...
import os,ctypes
from ctypes import CDLL,c_char,c_int, c_uint, c_bool, c_float, POINTER, byref


# # open dll
# sPath = os.path.dirname(os.path.abspath(__file__))
# _lib = ctypes.CDLL(os.path.join(sPath, 'DLL', 'VNX_atten64'))
...
# fnLMS_InitDevice = getDllObject('fnLDA_InitDevice')
...
# fnLMS_SetAttenuation = getDllObject('fnLMS_SetAttenuation', [DEVID, c_int])
...
class VaunixAttenuator():
    ...
    def __init__(self):
        ...
        dll_path = os.path.join(base_path,'DLL', 'VNX_atten64')
        self.VxDLL = CDLL(dll_path)
        self.VxDLL['fnLDA_SetTestMode'](False)
    ...
    def getListofDevices(self):
        ...
        nDev = self.callfuncret('fnLDA_GetDevInfo', byref(devices))
        for n1 in range(nDev):
            nameBuffer = STRING(b' '*32)
            self.callfuncret('fnLDA_GetModelNameA',devices[n1], nameBuffer)
            serial = int(self.callfuncret('fnLDA_GetSerialNumber',devices[n1]))
            d = dict()
    ...
    def getModelName(self,devID):
        ...
        self.callfuncret('fnLDA_GetModelNameA',devID,modelName)
    ...
    def initDevice(self,serial):
        ...
        self.DeviceID = DEVID(lDev[indx]['device_id'])
        # self.DeviceID = devID
