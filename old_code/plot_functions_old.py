...
def pulse_plot1d(sequence,x_vector,y_vector,dt=0.01,qubitDriveFreq=3.8e9,amplitude_hd=1,AC_pars=[0,0],RT_pars=[0,0],pi2Width=50e-9,fitting = 1, plot=1,save_fig = 0,iteration=1):
    '''
    ...
    if sequence == "rabi":
        ...
        period = 1e3/(extract_freq(x_vector*1e3, abs_camplitude, dt))
        # period = 80
        ...
        offset = np.mean(abs_camplitude)
        f = 1e-6*extract_freq(x_vector, abs_camplitude,dt)
    ...
    if plot == 1:
        if sequence == "rabi":
            ...
            textstr = '$\omega_d$ = %.4f GHz\n$A_d$ = %.2f V\n$T_{\pi/2}$ = %.1f ns\nAC $\mu$ = %.3f V'%(qubitDriveFreq*1e-9,amplitude_hd,round(pi_pulse/2,1),AC_pars[0])
            plt.gcf().text(1, 0.25, textstr, fontsize=14)
        elif sequence == "ramsey":
            fig = plt.figure()
            ax = fig.add_subplot(111)
            ...
            #     textstr = '$T_{\pi/2}$=%.1f ns\n$\omega_d$ = %.4f GHz\n$A_d$ = %.2f V\n$\Delta$=%.2f MHz\n$T_2^*$=%.2f $\mu$s\n$\mu$ = %.3f V\n$\sigma$ = %.3f V\n$B_0$ = %.2f V\n$\\tau_k$ = %.2f $\mu s$'%(pi2Width*1e9,qubitDriveFreq*1e-9,amplitude_hd,0,best_vals[1],AC_pars[0],AC_pars[1],RT_pars[0],RT_pars[1])
            # elif f>=0.1:
            ax.plot(x_vector,ramsey(x_vector, best_vals[0], best_vals[1], best_vals[2],best_vals[3],best_vals[4]),'r')
            textstr = '$T_{\pi/2}$=%.1f ns\n$\omega_d$ = %.4f GHz\n$A_d$ = %.2f V\n$\Delta$=%.3f MHz\n$T_2^*$=%.2f $\mu$s\n$\mu$ = %.3f V\n$\sigma$ = %.3f V\n$B_0$ = %.2f V\n$\\tau_k$ = %.2f $\mu s$'%(pi2Width*1e9,qubitDriveFreq*1e-9,amplitude_hd,best_vals[1],best_vals[3],AC_pars[0],AC_pars[1],RT_pars[0],RT_pars[1])
            ...
            ax.set_xlabel('Pulse Separation ($\mu$s)')
            ax.plot(x_vector,decay(x_vector, best_vals[0], best_vals[1], best_vals[2]),'r')
            textstr = '$T_{\pi/2}$=%.1f ns\n$\omega_d$ = %.4f GHz\n$A_d$ = %.2f V\n$T_2$=%.2f $\mu$s\n$\mu$ = %.3f V\n$\sigma$ = %.3f V\n$B_0$ = %.2f V\n$\\tau_k$ = %.2f $\mu s$'%(pi2Width*1e9,qubitDriveFreq*1e-9,amplitude_hd,best_vals[1],AC_pars[0],AC_pars[1],RT_pars[0],RT_pars[1])
...
def fit_sweep(par1_arr,par2_arr,sweep='sweep_001',plot=1,Tmax=5e-6,Tmax_b=10e-6):
    ...
    for i in range(len(par2_arr)):
        for j in range(len(par1_arr)):
            ...
            detun_b_arr[j,i] , T2_b_arr[j,i], error = pulse_plot1d(sequence='ramsey', x_vector=t_b, y_vector=np.mean(data_b,axis=0),dt=Tmax_b/data_b.shape[1],plot=plot, AC_pars=[0.6,0])
            end = time.time()
