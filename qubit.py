...
from plotting import plot_mixer_opt, power_plot, plot_coherence
...
class qubit():
    ...
    def zi_init(self, qa_delay=900):
        ...
        '''
        awg_setting = [
            # sets clock reference to external 10MHz
            ['/dev8233/system/clocks/referenceclock/source', 1],
            # sets grouping mode to 2x2
            ['/dev8233/system/awg/channelgrouping', 0],
            ['/dev8233/sigouts/*/on', 1],  # turn on outputs 1 and 2
            ['/dev8233/sigouts/*/range', 2],  # sets range of awg
            # enables oscillator control via AWG (needed so we can resetOscPhase via sequence program)
            ['/dev8233/system/awg/oscillatorcontrol', 1],
            # sets the oscillator freq
            ['/dev8233/oscs/0/freq', self.qb_pars['qb_IF']],
            ...
            # Output 1 modulated with Sine 1
            ['/dev8233/awgs/0/outputs/0/modulation/mode', 3],
            # Output 2 modulated with Sine 2
            ['/dev8233/awgs/0/outputs/1/modulation/mode', 4],
            ['/dev8233/sines/0/phaseshift', 0],   # Sine 1 phase
            ['/dev8233/sines/1/phaseshift', 180 + \
                self.qb_pars['qb_mixer_imbalance'][1]],   # Sine 2 phase
            # ['/dev8233/sines/0/phaseshift' , 90],   # Sine 2 phase
            # sets the marker 1 channel output
            ['/dev8233/triggers/out/0/source', 4]
        ]
        print('Applying initial settings to HDAWG')
        self.IQ_imbalance(
            g=self.qb_pars['qb_mixer_imbalance'][0], phi=self.qb_pars['qb_mixer_imbalance'][1])
        ...
        qa_setting = [
            ...
            # enables jumbo frames for faster connection | make sure you enable jumbo frames in windows network settings (how-to link:https://elements.tv/blog/achieve-maximum-ethernet-performance/)
            ['/dev2528/system/jumbo', 1],
            ['/dev2528/qas/0/integration/sources/0', 0],  # sets channel mapping
            # ['/dev2528/awgs/0/outputs/0/modulation/mode' , 0],
            # sets delay between trigger receive and start of integration
            ['/dev2528/qas/0/delay', round(qa_delay*1.8e9)],
            # 0 for standard (4096 samples max), 1 for spectroscopy
            ['/dev2528/qas/0/integration/mode', 0],
            # sets the oscillator freq
            ['/dev2528/oscs/0/freq', self.qb_pars['rr_IF']],
            ['/dev2528/qas/0/integration/length', 4096],
            # 0 for trigger input ch 1, 7 for internal triggering (QA AWG -> QA)]
            ['/dev2528/qas/0/integration/trigger/channel', 7],
            ['/dev2528/qas/0/monitor/trigger/channel', 7],
            ['/dev2528/qas/0/result/mode', 0],  # sets averaging to cyclic
            ['/dev2528/qas/0/result/reset', 1],
            # sets the digital trigger signal 1 input channel to physical input channel 3
            ['/dev2528/awgs/0/auxtriggers/0/channel', 2],
        ]
    ...
    def setup_active_reset(self):
        ...
        # select trigger sources
        # QA result trigger. The QA sends a trigger to HDAWG Ch. 1 when measurement is done.
        self.qa.setInt('/dev2528/triggers/out/0/source', 74)
        # QA result. Sends trigger to HDAWG based on the measurement result.
        self.qa.setInt('/dev2528/triggers/out/1/source', 64)
        ...
        self.qa.setDouble('/dev2528/qas/0/thresholds/0/level',
                          self.qb_pars['threshold'])
        self.qa.sync()

    def pulsed_rr_spec_setup(self):
        '''Setup UHFQA for pulsed resonator spectroscopy.'''
        self.qa.setInt('/dev2528/awgs/0/time',
                       3)  # sets AWG sampling rate to 225 MHz
        self.setup_rr_spec()

    def pulsed_qb_spec_setup(self):
        '''Setup HDAWG and UHFQA for pulsed spectroscopy'''
        self.awg.set('/dev8233/awgs/0/outputs/*/modulation/mode',
                     0)  # run in homodyne mode
        # sets AWG sampling rate to 600 MHz
        self.awg.setInt('/dev8233/awgs/0/time', 2)
        self.setup_awg()
        self.setup_qa_awg()
    ...
    def single_shot_setup(self):
        ...
        # sets AWG sampling rate to 2.4 GHz
    ...
    def single_shot(self, theta, device_name, qb='', save_data=True):
        ...
        # tells the API where to look for data in the QA
        sweep_data, paths = self.create_sweep_data_dict()
        data_pi = []
        data_OFF = []

        self.qa_result_reset()  # reset the QA result unit and clear errors
        # stops HDAWG (in case it was still executing some other experiment)
        self.enable_awg(self.awg, enable=0)
        # setups the QA for data integration.
        ...
        bt = time.time()
        # starts the qubit manipulation sequence
        self.enable_awg(self.awg, enable=1)
        # transfers data from the QA result to the API
        data = self.acquisition_poll(
            paths, result_length, timeout=10*result_length*self.qb_pars['qubit_reset_time'])
        et = time.time()
        duration = et-bt
        print(f'Measurement time: {duration:.1f} s')

        # seperate OFF/pi data (no averaging)
        data_OFF = np.append(data_OFF, [data[paths[0]][k]
                             for k in utils.even(len(data[paths[0]]))])/4096
        data_pi = np.append(data_pi, [data[paths[0]][k]
                            for k in utils.odd(len(data[paths[0]]))])/4096
    ...
    def rr_spectroscopy(self, freqs, device_name, qb='', save_data=True):
        ...
        with tqdm(total=len(freqs)) as progress_bar:
            for f in freqs:
                # updates frequency
                instfuncs.set_LO(self.exp_pars['element'], f*1e9, sweep=True)
        ...
        norm = self.qa.get('/dev2528/qas/0/integration/length')[
            'dev2528']['qas']['0']['integration']['length']['value'][0]
        ...
        power_data = np.sqrt(I_data*I_data.conjugate() +
                             Q_data*Q_data.conjugate())
    ...
    def pulsed_exp(self, verbose=1, qb='', device_name='', check_mixers=False, save_data=True, ssb=False):
        ...
        if self.exp_pars['exp'] == 'p-rabi' or self.exp_pars['exp'] == 'z-gate':
            self.x0, self.xmax, self.dx, x_array, self.n_steps = utils.generate_xarray(
                self.exp_pars)
            self.n_points = 100
        else:
        ...
        self.config_qa(result_length=self.n_steps, source=source, ssb=ssb)
        self.qa.sync()
        ...
        print('Estimated Measurement Time (with/without active reset): %.3f/%.3f sec' %
        ...
        data = self.acquire_data(paths, self.n_steps, timeout=4*timeout)
        ...
        norm = self.qa.get('/dev2528/qas/0/integration/length')[
            'dev2528']['qas']['0']['integration']['length']['value'][0]
        # normalizes the data according to the integration length
        data = sweep_data[paths[0]][0:]/norm
    ...
    def tomography_calibration(self, device_name='', qb='', check_mixers=False, verbose=True, save_data=False):
        ...
        # setup QA Result unit
        # configure UHFQA result unit, source = 2 means data is rotated
        self.config_qa(result_length=3*(self.n_steps+1), source=source, ssb=False)
        self.qa.sync()
        ...
        temp = self.acquire_data(
            paths, result_length=3*(self.n_steps+1), timeout=3.5*timeout)
        end_meas = time.time()
        print('\nmeasurement duration: %.1f s' % (end_meas-str_meas))
    ...
    def coherence_stabilization(self, device_name='', qb='', calibrate=False, verbose=True, save_data=False):
        ...
        source = 2  # display source: rotation
        # update qubit IF frequency
        # self.update_qb_value('qb_IF', self.exp_pars['qubit_drive_freq']-self.qb_pars['qb_LO'])
        self.awg.setDouble('/dev8233/oscs/0/freq', self.qb_pars['qb_IF'])
        if calibrate:
            self.min_leak(
                inst=self.awg, f_LO=self.qb_pars['qb_LO'], mixer='qubit', cal='lo', plot=True)
            self.min_leak(
                inst=self.qa, f_LO=self.qb_pars['rr_LO'], mixer='rr', cal='lo', plot=True)
            self.min_leak(
                inst=self.awg, f_LO=self.qb_pars['qb_LO'], f_IF=self.qb_pars['qb_IF'], cal='ssb', amp=0.3, threshold=-50, span=0.5e6)
        ...
        #t = np.linspace(self.x0,self.xmax,data.shape[1]-2)
            ...
            for j in range(self.exp_pars['n_realizations']):
                for i, st in enumerate(['X', 'Y', 'Z']):
                    ...
                    norm = self.qa.get('/dev2528/qas/0/integration/length')[
                        'dev2528']['qas']['0']['integration']['length']['value'][0]
                    # normalizes the data according to the integration length
                    data[i, :, j] = sweep_data[paths[0]][:]/norm

                    # self.qa_result_reset()
                    # stops the readout sequence
                    self.enable_awg(self.qa, enable=0)

                    # compute offset and amplitude
                    offset[i] = (data[i, 0, j]+data[i, -1, j])/2
                    amp[i] = (data[i, 0, j]-data[i, -1, j])/2

                    # compute new threshold based on offset
                    self.qb_pars['threshold'] = offset[i]*4096
                    self.qa.setDouble(
                        '/dev2528/qas/0/thresholds/0/level', self.qb_pars['threshold'])
                ...
                # Recalibrate coordinates using offset and amplitudes
                calib_states = self.cal_coord_new(offset, amp)
                v_b[:, :, j] = utils.compute_bloch(data[:, :, j], calib_states)
                # self.exp_pars['threshold'] = offset*4096
                # self.qb_pars['threshold'] = offset_z*4096
                # self.qb_pars['threshold'] = np.mean(offset)*4096 #update active reset threshold
                ...
                #if np.mod(j+1,2)==0:
                   # v_b_temp = v_b[:, :, 0:j]
                  #  v_b_temp = np.mean(v_b_temp,2)
                  #  wfms = self.get_wfms()
                  #  plot_coherence(t,v_b_temp,wfms,self.exp_pars,self.qb_pars,self.wfm_pars,calib_states)
                pbar.update(1)
...
#%% Coherence Stabiization
    def setup_coherence_stabilization(self, 
                                      device_name='', 
                                      qb='', 
                                      calibrate=False, 
                                      verbose=True):
        '''
        Sets up and enables Quantum Analyzer and HDAWG.
        Converts waveform parameters and sets up output arrays.
        ...
        ## Calculate and print out estimated time for the measurement
        exp_dur = self.calc_timeout()
        print('Estimated Measurement Time (with/without active reset): %.3f/%.3f sec' %
              (int(1/8*exp_dur), exp_dur))
        ## Calculate timeout (how long it will wait before returning an error)
        ...
        # data = np.zeros((3, self.n_steps+2, self.exp_pars['n_realizations']))
        data = np.zeros((3*self.n_steps+2, self.exp_pars['n_realizations']))
        v_b = np.zeros((3, self.n_steps, self.exp_pars['n_realizations']))
        #offset = np.zeros(3)
        #amp = np.zeros(3)
        #if save_data:
        filepath = self.create_datafile(qb, device_name,self.exp_pars)
        #else:
        #    pass
        return data, v_b, timeout, self.n_steps, filepath


    def coherence_stabilization_single(self,  timeout, n_steps, filepath, device_name='', qb='',  calibrate=False,  verbose=True):
        '''Runs a single instance of coherence stabilization'''
        ...
        # runs the drive sequence
        self.enable_awg(self.awg, enable=1)
        temp = self.acquire_data(paths, 3*self.n_steps+2, timeout=timeout) # retrieve data from UHFQA
        end_meas = time.time()
        print('\n Measurement Duration: %.1f s' %
                          (end_meas-start_meas))
        for path, samples in temp.items():
            ...
            norm = self.qa.get('/dev2528/qas/0/integration/length')[
                        'dev2528']['qas']['0']['integration']['length']['value'][0]
                    # normalizes the data according to the integration length
            data[:] = sweep_data[paths[0]][:]/norm

                    # self.qa_result_reset()
            # stops the readout sequence
            self.enable_awg(self.qa, enable=0)

            # compute offset and amplitude take the first and last value of data array
            offset = (data[0]+data[-1])/2
            amp = (data[0]-data[-1])/2

                

            # compute new threshold based on offset
            self.qb_pars['threshold'] = offset*4096
            # Update Threshold
            self.qa.setDouble(
                        '/dev2528/qas/0/thresholds/0/level', self.qb_pars['threshold'])
                ...
                # Recalibrate coordinates using offset and amplitudes
            calib_states = self.cal_coord(offset, amp)  
            #going back to old cal_coord since we only have one offset 
                #and one amp value
                #v_b[:, :, j] = utils.compute_bloch(data[:, :, j], calib_states)
            #print(data)
            #print(data.shape)
            # take x component of data with computed bloch vector: (this replaces 
            # compute_bloch function)

            # start  with 1 because we skip the first point in the data array.
            v_b[0,:]= 1 - 2 * (calib_states[2] - data[1:-1:3]) / calib_states[3]
            # take y component of data with computed bloch vector:
            v_b[1,:] = 1 - 2 * (calib_states[2] - data[2:-1:3]) / calib_states[3]
            # take y component of data with computed bloch vector:
            v_b[2,:] = 1 - 2 * (calib_states[2] - data[3:-1:3]) / calib_states[3]

            try:
                #self.save_data(filepath, v_b[:,:])
                self.save_data(filepath, v_b)
            except:
                pass
                #pbar.update(1)
        ...
        return wfms, results, v_b, calib_states, n_steps
    # %% setup_HDAWG
    ...
    def setup_awg(self):
        ...
        # sets AWG sampling rate to 600 MHz
        self.awg.setInt('/dev8233/awgs/0/time',
                        int(np.log2(2.4e9/self.exp_pars['fsAWG'])))
        ...
        # setup constants
        seqs.setup_seq_pars(
            self.sequence, self.exp_pars['exp'], self.exp_pars, self.qb_pars, self.n_steps)
        ...
        if self.exp_pars['exp'] == 'coherence-stabilization' or self.exp_pars['exp'] == 'T1' or self.exp_pars['exp'] == 'ramsey':
            ...
            self.sequence = seqs.setup_waveforms(
                self.sequence, {}, self.exp_pars, self.qb_pars, n_points=self.n_points)
        # setup command table
  ...
  # %% set up mixer calib         
    ...
    def setup_qa_awg(self, ssb=False):
        ...
        self.qa_sequence.constants['delay'] = round(
            self.qb_pars['cav_resp_time']/(4.4e-9))
        ...
        with self.qa_ses.set_transaction():
            ...
            self.qa_ses.awgs[0].write_to_waveform_memory(
                self.qa_sequence.waveforms)

        # sets the source of digital trigger 1 to be the signal at trigger input 3 (back panel)
        self.qa.setInt('/dev2528/awgs/0/auxtriggers/0/channel', 2)
    ...
    def setup_rr_spec(self):
        ...
        self.qa_sequence.constants['n_avg'] = self.n_steps
        self.qa_sequence.constants['rr_reset_time'] = utils.roundToBase(
            self.exp_pars['rr_reset_time']*225e6)
        self.qa_sequence.constants['delay'] = round(
            self.qb_pars['cav_resp_time']/(4.4e-9))
    ...
    def config_qa(self, result_length=1, source=7, ssb=False):
        ...
        # else:
        # 0 for standard (4096 samples max), 1 for spectroscopy
        self.qa.setInt('/dev2528/qas/0/integration/mode', 0)
        self.qa.setInt('/dev2528/qas/0/integration/length', L)
        self.qa.setInt('/dev2528/qas/0/bypass/crosstalk',
                       bypass_crosstalk)  # No crosstalk matrix
        self.qa.setInt('/dev2528/qas/0/bypass/deskew',
                       1)  # No crosstalk matrix
        # self.qa.setInt('/dev2528/qas/0/bypass/rotation'.format(device), 1)   #No rotation
        if ssb:
            x = np.linspace(0, self.qb_pars['integration_length'], round(
                self.qb_pars['integration_length']*base_rate))
            weights_I = np.sin(2*np.pi*self.qb_pars['rr_IF']*x)
            weights_Q = np.zeros(
                round(self.qb_pars['integration_length']*base_rate))
        else:
            weights_I = weights_Q = np.ones(L)
        # weights_Q = np.zeros(round(integration_length*base_rate))
        self.qa.setVector(
            '/dev2528/qas/0/integration/weights/0/real', weights_I)
        self.qa.setVector(
            '/dev2528/qas/0/integration/weights/0/imag', weights_Q)
        self.qa.sync()
        # 0 for trigger input ch 1, 7 for internal triggering (QA AWG -> QA)
        self.qa.setInt('/dev2528/qas/0/integration/trigger/channel', 7)
        ...
        # else:
        self.qa.setInt('/dev2528/qas/0/monitor/averages',
                       self.exp_pars['n_avg'])
        self.qa.setInt('/dev2528/qas/0/result/averages',
                       self.exp_pars['n_avg'])
        self.qa.setInt('/dev2528/qas/0/monitor/length', L)
        self.qa.setComplex('/dev2528/qas/0/rotations/0',
                           np.exp(1j*self.qb_pars['IQ_rotation']*pi/180))
        ...
        # 2 -> source = rotation | 7 = integration
        self.qa.setInt('/dev2528/qas/0/result/source', source)
    ...
    def acquisition_poll(self, paths, num_samples, timeout):
        ...
        while t < timeout and not all(gotem.values()):
            ...
            dataset = self.qa.poll(
                poll_length, poll_timeout, poll_flags, poll_return_flat_dict)
        ...
        if not all(gotem.values()):
            ...
            raise Exception(
                'Timeout Error: Did not get all results within {:.1f} s!'.format(timeout))
    ...
    def pull_wfm(self, sweep_name, nu, tauk, sequence='ramsey'):
        ...
        path = "E:\\generalized-markovian-noise\\%s\\sweep_data\\%s\\%s\\noise_instances" % (
            'CandleQubit_6', sequence, sweep_name)
        filename = 'nu_%d_Hz_tau_%d_ns.csv' % (round(nu*1e3), round(tauk*1e3))
        # filename = 'RTN_tau_%d_ns.csv' %(round(tau*1e3))
        print(os.path.join(path, filename))
        noise_realizations = np.loadtxt(os.path.join(
            path, filename), dtype=float, delimiter=',')
    ...
    def create_wfms(self, sequence="ramsey", n_points=1000, Tmax=5e-6):
        ...
        if sequence == 'rabi':
            fsAWG = 2.4e9
            gauss_pulse = gaussian(utils.roundToBase(
                self.qb_pars['gauss_len']*self.exp_pars['fsAWG']), utils.roundToBase(self.pars['gauss_len']/5*self.exp_pars['fsAWG']))
            drive_pulse = np.concatenate((gauss_pulse[:len(gauss_pulse)/2], np.ones(
                utils.roundToBase(Tmax*fsAWG)), gauss_pulse[len(gauss_pulse)/2+1:]))
            # save wfm to file
    ...
    def calc_timeout(self):
        ...
        t = (self.n_steps*(self.dx /
             self.exp_pars['fsAWG']+self.qb_pars['qubit_reset_time']))*self.exp_pars['n_avg']
        return t
    ...
    def init_arrays(numRealizations=128, interval=2, n_pointsBackground=200, n_points=200):
        ...
        bData = np.zeros((int(numRealizations/interval),
                         n_pointsBackground), dtype=float)
...
#         return power
    ...
    #         self.sa.setValue('Threshold',threshold)
        ...
        try:
            ...
            signal = self.sa.getValue('Signal')
            signal = signal['y']
        power = np.max(signal)
    #     # signal = sa_get_sweep_64f(self.sa)['max']
    #     signal = self.sa.getValue('Signal')['y']
    ...
    def min_leak(self, inst, f_LO=1e9, f_IF=0, mixer='qubit', threshold=-50, span=0.5e6, cal='lo', amp=0.2, measON=False, plot=False):
        ...
        OFF_power = self.get_power(
            fc=f_LO, threshold=-20, config=True, plot=True, span=span)
        ...
        for i in range(len(vStart)):
            vStart[i] = inst.get(
                f'/{device}/sigouts/{i}/offset')[f'{device}']['sigouts'][f'{i}']['offset']['value']
            inst.sync()
            ...
            for i, V1 in enumerate((VoltRange1)):
                for j, V2 in enumerate((VoltRange2)):
                    ...
                    power_data[i, j] = self.get_power(
                        fc=f_LO, plot=False, config=False)
                    progress_bar.update(1)
        ...
        print(
            f'optimal I_offset = {round(opt_I*1e3,1)} mV, optimal Q_offset = {round(1e3*opt_Q,1)} mV')
        ...
        if plot:
            plot_mixer_opt(VoltRange1, VoltRange2, power_data,
                           cal='LO', mixer=mixer, fc=f_LO)
    ...
    def suppr_image(self, inst, mixer='qubit', threshold=-50, f_LO=3.875e9, f_IF=50e6, amp=0.2,
        ...
        OFF_power = self.get_power(
            fc=f_im, threshold=-20, plot=True, span=span)
        ...
        threshold = -40
        print(
            f'Image Sideband leakage is {OFF_power:.1f} dBm\nSetting SA threshold to {threshold:.1f} dBm')
        start = time.time()
        ...
        if plot:
            plot_mixer_opt(ampArr, phiArr,  power_data,
                           cal='SB', mixer='qubit', fc=f_im)
    ...
    def sort_data(self, data, paths):
        ...
        if self.exp_pars['exp'] == 'spectroscopy':
            ...
            if self.exp_pars['on_off']:
                data_OFF = []
                data_OFF = np.append(data_OFF, np.mean(
                    [data[paths[0]][k] for k in utils.even(len(data[paths[0]]))]))
                data_ON = np.append(data_ON, np.mean(
                    [data[paths[0]][k] for k in utils.odd(len(data[paths[0]]))]))
            else:
    ...
    def cal_coord(self, offset, amp):
        ...
        #print(
        #    f'Ground state voltage: {grn*1e3:.1f} mV\nPeak-to-Peak Amplitude: {vpp*1e3:.1f} mV')

        return px, py, grn, vpp

    def cal_coord_new(self, offset, amp):
        vpp = np.zeros(len(amp))
        px = offset[0]+amp[0]
        py = offset[1]+amp[1]
        grn = offset[2]+amp[2]
        ...
        return px, py, grn, vpp[0], vpp[1], vpp[2]
    ...
    def acquire_data(self, paths, result_length, timeout):
        ...
        while kk < 101:
            try:
                # transfers data from the QA result to the API for this frequency point
                data = self.acquisition_poll(paths, result_length, timeout)
                # data = self.get_data(wave_data_captured,result_length,timeout=10)
                # print(data)
                break
                ...
                print(
                    'Error! Unable to retrieve data from Quantum Analyzer. Trying again. Might have to restart QA')
                ...
                try:
                    ...
                    print('Lost connection to UHFQA! Attempting to reconnect')
                    self.qa, device_id, _ = create_api_session(
                        'dev2528', api_level=6)
                    self.qa_ses = self.session.connect_device('DEV2528')
                    self.awg, device_id, _ = create_api_session(
                        'dev8233', api_level=6)
                    self.hdawg_core = self.session.connect_device('DEV8233')
                    self.qa_result_reset()
    ...
    def upload_to_awg(self, ct):
        ...
        with self.hdawg_core.set_transaction():
            ...
            try:
                ...
                print(self.sequence)
            # upload waveforms
            self.hdawg_core.awgs[0].write_to_waveform_memory(
                self.sequence.waveforms)
    ...
    def IQ_imbalance(self, g, phi, amp=1):
        ...
        if np.abs(g*amp*np.sin(phi)) > 1 or np.abs(g*amp*np.cos(phi)) > 1:
            corr = np.array(
                [float(amp * x) for x in [1/g, 0, np.sin(phi), -np.cos(phi)]]).reshape(2, 2)
        else:
            corr = np.array(
                [float(amp * x) for x in [1, 0, g*np.sin(phi), -g*np.cos(phi)]]).reshape(2, 2)

        for i in range(2):
            for j in range(2):
                self.awg.set(
                    f'/dev8233/awgs/0/outputs/{j}/gains/{i}', corr[i, j])
    ...
    def create_datafile(self, qb, device_name,exp_pars):
    ...
    def retrieve_datafile(self, filepath):
        filepath = 'D:\\coherence-stabilization\\WM1\qb6\\coherence-stabilization-data\\coherence-stabilization_data_1154.csv'
        df = pd.read_csv(filepath,skiprows =[0,1,2,3],usecols=[1,2],header = None)
        #print(df)
        v_data =df.to_numpy()
    ...
    def calc_amp(self, device, amp):
        ...
        awg_range = self.awg.get(
            f'/{device}/sigouts/0/range', flat=True)[f'/{device}/sigouts/0/range']['value'][0]
        if 2*amp*awg_range > awg_range:
            raise ValueError(
                f'Waveform amplitude [{amp}] exceeds {device} range [{awg_range}]')
        else:
            amp = awg_range*amp/2
    ...
    def set_config(self, awg, qa):
        ...
        self.IQ_imbalance(
            awg, g=self.pars['qubit_mixer_imbalance'][0], phi=self.pars['qubit_mixer_imbalance'][1])

        awg_setting = [
            # sets the oscillator freq
            ['/dev8233/oscs/0/freq', self.pars['qubit_IF']],
    ...
    def make_wfm_file(self, filename, wfm_data):
        ...
        # save file where it can be called by the AWG sequence program
        np.savetxt(path+filename+"_wfm"+".csv", wfm_data, delimiter=",")
